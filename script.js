...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('agregarLista').onclick = mostrarMenuListas;
...
function mostrarSeccion(id) {
  ...
  if (id === 'parati') mostrarSeccionParaTi();
...
function mostrarResultadosConLogos(items, contenedorId) {
  ...
  const needsWrapper = ['peliculas','series','tendencias'].includes(contenedorId);
  let target = cont;
  if (needsWrapper) {
    const g = document.createElement('div'); g.className = 'grid';
    cont.appendChild(g); target = g;
  }
  items.forEach(item => {
    const html = tarjetaItemHTML(item); if (!html) return;
    const div = document.createElement('div'); div.classList.add('card');
    div.innerHTML = html; div.onclick = () => abrirModal(item);
...
function agregarResultadosConLogos(items, containerId) {
  ...
  const needsWrapper = ['peliculas','series','tendencias'].includes(containerId);
  const target = needsWrapper
    ? (container.querySelector('.grid') || (() => { const g=document.createElement('div'); g.className='grid'; container.appendChild(g); return g; })())
    : container;
  items.forEach(item => {
    const html = tarjetaItemHTML(item); if (!html) return;
    const div = document.createElement('div'); div.classList.add('card');
...
const AGENDA_PROVIDERS = {
  all:[8,337,1899,119,350,1773,531,63,149], netflix:[8], disneyplus:[337],
  hbomax:[1899], primevideo:[119], appletv:[350], skyshowtime:[1773],
  paramountplus:[531], filmin:[63], movistar:[149]
};
const AGENDA_PROVIDER_NAMES = {
  8:'Netflix',337:'Disney+',1899:'Max',119:'Prime Video',
  350:'Apple TV+',1773:'SkyShowtime',531:'Paramount+',63:'Filmin',149:'Movistar+'
};
const AGENDA_LOGOS = {
...
async function obtenerSeriesTMDB(providerIds,fechaInicio,fechaFin) {
  ...
  await Promise.all(providerIds.map(async pid=>{
    for(let page=1;page<=3;page++){
      try{
        const data=await(await fetch(`${BASEURL}discover/tv?api_key=${APIKEY}&language=es-ES&watch_region=ES&with_watch_providers=${pid}&air_date.gte=${fechaInicio}&air_date.lte=${fechaFin}&sort_by=first_air_date.asc&page=${page}`)).json();
        if(!(data.results||[]).length) break;
        data.results.forEach(s=>{if(!vistos.has(s.id)){vistos.add(s.id);s._provider_id=pid;todas.push(s);}});
      }catch{break;}
    }
  }));
  return todas;
...
async function obtenerEpisodiosMes(serieId,fechaInicio,fechaFin){
  try{
    const det=await(await fetch(`${BASEURL}tv/${serieId}?api_key=${APIKEY}&language=es-ES`)).json();
    const eps=[];
    await Promise.all((det.seasons||[]).filter(s=>s.season_number>0).map(async t=>{
      try{
        const dt=await(await fetch(`${BASEURL}tv/${serieId}/season/${t.season_number}?api_key=${APIKEY}&language=es-ES`)).json();
        (dt.episodes||[]).forEach(ep=>{const f=ep.air_date||'';if(f&&f>=fechaInicio&&f<=fechaFin)eps.push({fecha:f,temporada:t.season_number,numero:ep.episode_number,titulo:ep.name||''});});
      }catch{}
    }));
    return eps.sort((a,b)=>a.fecha.localeCompare(b.fecha)||a.temporada-b.temporada||a.numero-b.numero);
  }catch{return[];}
}
...
async function cargarAgenda(reset=false){
  ...
  try{
    const ck=`agenda_v3_${filtrosAgenda.fecha}_${filtrosAgenda.plataforma}`;
    const cached=localStorage.getItem(ck);
    if(cached){const p=JSON.parse(cached);if(Date.now()-p.time<CACHE_TIME){todosLosItemsAgenda=p.data||[];ocultarLoader('agendaContainer');agendaItemsVisibles=0;renderAgendaLote(true);agendaCargando=false;return;}}
    const{hoy,dias}=getRangoAgenda();
    const fechaInicio=getDateISO(hoy),fechaFin=getDateISO(sumarDias(hoy,dias));
    const pids=AGENDA_PROVIDERS[filtrosAgenda.plataforma]||AGENDA_PROVIDERS.all;
    const series=await obtenerSeriesTMDB(pids,fechaInicio,fechaFin);
    const items=[];
    await Promise.all(series.map(async serie=>{
      ...
      const pName=AGENDA_PROVIDER_NAMES[serie._provider_id]||'Streaming';
      const logo=AGENDA_LOGOS[pName]||null;
      const porFecha={};
      ...
      Object.entries(porFecha).forEach(([fecha,eps])=>{
        const total=eps.length;let episodioTexto,badge;
        ...
        items.push({id:`tmdb-${serie.id}-${fecha}`,tmdb_id:serie.id,titulo:serie.name||'Sin titulo',
          plataforma:pName,fecha,hora:'--:--',poster,resumen:(serie.overview||'').substring(0,200),
          vote_average:serie.vote_average||0,episodio:episodioTexto,badge,logoCanal:logo,
          plataformas:[{provider_name:pName,logo_url:logo}]});
      });
    }));
    todosLosItemsAgenda=items.sort((a,b)=>a.fecha.localeCompare(b.fecha));
    localStorage.setItem(ck,JSON.stringify({time:Date.now(),data:todosLosItemsAgenda}));
    ocultarLoader('agendaContainer');agendaItemsVisibles=0;renderAgendaLote(true);
  }catch(e){console.error(e);ocultarLoader('agendaContainer');mostrarNotificacion('Error cargando agenda','error');}
...
function renderAgendaLote(reset=false){
  ...
  if(!todosLosItemsAgenda.length){container.innerHTML='<p style="text-align:center;padding:2rem;color:#ffd700;">No hay resultados</p>';stats.innerHTML='Sin resultados';return;}
  ...
  Object.keys(agrupado).sort().forEach(fecha=>{
    ...
    lista.forEach(item=>{
      ...
      const bClass=item.badge==='Temporada'?'nueva':item.badge==='Estreno'?'estreno':item.badge==='Doble'?'doble':'episodio';
      const pHTML=item.poster?`<img src="${item.poster}" alt="${escapeHtml(item.titulo)}" class="agenda-poster" onerror="this.style.display='none'">`:`<div class="agenda-poster agenda-poster-fallback">?</div>`;
      const lHTML=item.logoCanal?`<div class="agenda-plataformas"><img src="${item.logoCanal}" class="agenda-platform-logo" title="${escapeHtml(item.plataforma)}" onerror="this.style.display='none'"></div>`:`<div class="agenda-plataformas"><span>${escapeHtml(item.plataforma)}</span></div>`;
      const rHTML=item.resumen?`<p class="agenda-resumen">${escapeHtml(item.resumen)}</p>`:'';
      card.innerHTML=`<div class="agenda-poster-wrap">${pHTML}</div><div class="agenda-info"><div class="agenda-topline"><h4 class="agenda-titulo">${escapeHtml(item.titulo)}</h4><span class="agenda-badge agenda-badge-${bClass}">${escapeHtml(item.badge)}</span></div><div class="agenda-episodio">${escapeHtml(item.episodio||'')}</div><div class="agenda-meta"><span>${escapeHtml(item.plataforma)}</span><span>${item.vote_average?Number(item.vote_average).toFixed(1):'N/A'}</span></div>${lHTML}${rHTML}</div><div class="agenda-accion"><button class="agenda-btn" onclick="abrirModalAgenda(${item.tmdb_id})">Ver</button></div>`;
...
function aplicarFiltrosAgenda(){
  ...
  Object.keys(localStorage).filter(k=>k.startsWith('agenda_v3')).forEach(k=>localStorage.removeItem(k));
  todosLosItemsAgenda=[];agendaItemsVisibles=0;cargarAgenda(true);
}

async function abrirModalAgenda(tmdbId){
  try{
    const show=await(await fetch(`${BASEURL}tv/${tmdbId}?api_key=${APIKEY}&language=es-ES`)).json();
    const item=await enriquecerConPlataformasTMDb({id:show.id,titulo:show.name,overview:show.overview||'',
      poster:show.poster_path?`https://image.tmdb.org/t/p/w300${show.poster_path}`:null,
      poster_path:show.poster_path,vote_average:show.vote_average||0,fecha:show.first_air_date||'',tipo:'tv'},'tv');
    abrirModal(item);
...
// ============================================
// MENU LISTAS - dropdown visual (sin prompt)
// ============================================
function getListas(){
  const raw=localStorage.getItem('listas');
  if(raw){try{return JSON.parse(raw);}catch{}}
  const antigua=localStorage.getItem('miLista');
  if(antigua){try{const it=JSON.parse(antigua);const ls=[{id:'default',nombre:'Mi Lista',items:it,creada:new Date().toISOString()}];localStorage.setItem('listas',JSON.stringify(ls));return ls;}catch{}}
...
function mostrarMenuListas(){
  if(!itemActual)return;
  const id=itemActual.tmdb_id||itemActual.id;
  // Si ya esta, preguntar si eliminar
  if(estaEnAlgunaLista(id)){
    if(confirm('Ya esta en tu lista. Quieres eliminarlo?')){
      const listas=getListas();
      listas.forEach(l=>{l.items=l.items.filter(i=>String(i.id)!==String(id));});
  ...
  // Eliminar menu anterior si existe
  const anterior=document.getElementById('menuListas');
  if(anterior)anterior.remove();
  const listas=getListas();
  // Si solo hay una lista, agregar directamente
  if(listas.length===1){
    agregarALista(listas[0].id);return;
  }
  // Mostrar dropdown visual
  const menu=document.createElement('div');
  menu.id='menuListas';
  menu.style.cssText='position:fixed;z-index:9999;background:#1a1a2e;border:2px solid #e74c3c;border-radius:14px;padding:1rem;min-width:220px;box-shadow:0 10px 30px rgba(0,0,0,0.7);';
  const btn=document.getElementById('agregarLista');
  const rect=btn?btn.getBoundingClientRect():{top:window.innerHeight/2,left:window.innerWidth/2};
  menu.style.top=(rect.top-listas.length*52-20)+'px';
  menu.style.left=Math.min(rect.left,window.innerWidth-240)+'px';
  const titulo=document.createElement('p');
  titulo.style.cssText='color:#ffd700;font-weight:bold;margin-bottom:0.8rem;font-size:0.9rem;';
  titulo.textContent='Agregar a lista:';
  menu.appendChild(titulo);
  listas.forEach(lista=>{
    const item=document.createElement('button');
    item.style.cssText='display:flex;justify-content:space-between;align-items:center;width:100%;padding:0.7rem 1rem;margin-bottom:6px;background:rgba(255,255,255,0.07);border:1px solid rgba(255,255,255,0.1);border-radius:10px;color:white;cursor:pointer;transition:all 0.2s;font-size:0.9rem;';
    item.innerHTML=`<span>${escapeHtml(lista.nombre)}</span><small style="color:#ffd700">${lista.items.length}</small>`;
    item.onmouseover=()=>{item.style.background='rgba(231,76,60,0.25)';item.style.borderColor='#e74c3c';};
    item.onmouseout=()=>{item.style.background='rgba(255,255,255,0.07)';item.style.borderColor='rgba(255,255,255,0.1)';};
    item.onclick=()=>{agregarALista(lista.id);menu.remove();};
    menu.appendChild(item);
  });
  const crearBtn=document.createElement('button');
  crearBtn.style.cssText='width:100%;padding:0.6rem;margin-top:4px;background:transparent;border:1px dashed rgba(255,255,255,0.3);border-radius:10px;color:#b0b0b0;cursor:pointer;font-size:0.85rem;';
  crearBtn.textContent='+ Nueva lista';
  crearBtn.onclick=()=>{crearListaModal();menu.remove();};
  menu.appendChild(crearBtn);
  document.body.appendChild(menu);
  // Cerrar al hacer click fuera
  setTimeout(()=>{document.addEventListener('click',function handler(e){if(!menu.contains(e.target)&&e.target!==btn){menu.remove();document.removeEventListener('click',handler);}});},50);
}

function agregarALista(listaId){
  const listas=getListas();
  ...
  guardarListas(listas);
  mostrarNotificacion(`Agregado a "${lista.nombre}"`, 'success');
  actualizarBotonLista();
}

function crearListaModal(){
...
// ── Reemplazar agregarMiLista para compatibilidad ──
function agregarMiLista(){mostrarMenuListas();}

// ── Reemplazar cargarMiLista para multilistas ──
function cargarMiLista(){
  const listas=getListas();
  const container=document.getElementById('miLista');
  container.innerHTML='';
  // Header con boton nueva lista
  const header=document.createElement('div');
  header.style.cssText='grid-column:1/-1;display:flex;gap:10px;align-items:center;flex-wrap:wrap;margin-bottom:1.5rem;';
  header.innerHTML='<h2 style="margin:0;flex:1">Mis Listas</h2><button class="btn-perfil" onclick="crearListaModal()" style="white-space:nowrap">+ Nueva lista</button>';
  container.appendChild(header);
  listas.forEach(lista=>{
    const seccion=document.createElement('div');
    seccion.style.cssText='grid-column:1/-1;margin-bottom:2.5rem;';
    const bar=document.createElement('div');
    bar.style.cssText='display:flex;align-items:center;gap:10px;margin-bottom:1rem;padding:0.8rem 1.2rem;background:rgba(255,255,255,0.06);border-radius:12px;border:1px solid rgba(255,255,255,0.08);';
    bar.innerHTML=`<h3 style="margin:0;flex:1;font-size:1.1rem">${escapeHtml(lista.nombre)} <span style="color:#ffd700;font-size:0.85rem">(${lista.items.length})</span></h3>`
      +`<button class="btn-perfil" style="padding:4px 12px;font-size:0.8rem" onclick="renombrarLista('${lista.id}')">Renombrar</button>`
      +`<button class="btn-perfil" style="padding:4px 12px;font-size:0.8rem;background:rgba(244,67,54,0.3);border-color:#f44336" onclick="eliminarLista('${lista.id}')">Eliminar lista</button>`
      +`<button class="btn-perfil" style="padding:4px 12px;font-size:0.8rem;background:rgba(33,150,243,0.3);border-color:#2196F3" onclick="compartirEstaLista('${lista.id}')">Compartir</button>`;
    seccion.appendChild(bar);
    if(!lista.items.length){
      const p=document.createElement('p');p.style.cssText='padding:1rem;color:#888;font-size:0.9rem;';p.textContent='Lista vacia';seccion.appendChild(p);
    }else{
      ...
      lista.items.forEach(item=>{
        ...
        const poster=item.poster_path.startsWith('http')?item.poster_path:`https://image.tmdb.org/t/p/w300${item.poster_path}`;
        const div=document.createElement('div');div.classList.add('card');
        div.innerHTML=`<img src="${poster}" loading="lazy" alt="${escapeHtml(item.title||'')}"><h4>${escapeHtml(item.title||item.name||'')}</h4><p>${item.vote_average?.toFixed?.(1)||'N/A'}</p><p>${formatDate(item.release_date)}</p><button class="btn-eliminar" onclick="eliminarItemLista('${item.id}','${lista.id}',event)">Eliminar</button>`;
        div.addEventListener('click',e=>{if(!e.target.classList.contains('btn-eliminar'))abrirModal(item);});
...
function renombrarLista(id){
  const listas=getListas();const lista=listas.find(l=>l.id===id);if(!lista)return;
  const nuevo=prompt('Nuevo nombre:',lista.nombre);if(!nuevo||!nuevo.trim())return;
...
function eliminarLista(id){
  ...
  guardarListas(listas.filter(l=>l.id!==id));mostrarNotificacion('Lista eliminada','success');cargarMiLista();
}

function eliminarItemLista(itemId,listaId,event){
  event.stopPropagation();
  if(!confirm('Eliminar este item?'))return;
  const listas=getListas();const lista=listas.find(l=>l.id===listaId);
...
// ── Eliminar de MI lista (compatibilidad con codigo viejo) ──
function eliminarDeMiLista(id,event){
  if(event)event.stopPropagation();
  if(!confirm('Eliminar este item?'))return;
  const listas=getListas();
  listas.forEach(l=>{l.items=l.items.filter(i=>String(i.id)!==String(id));});
  guardarListas(listas);cargarMiLista();mostrarNotificacion('Eliminado','success');
}

// ============================================
// COMPARTIR - formato bonito + Web Share API
// ============================================
async function compartirEstaLista(listaId){
  const listas=getListas();
  ...
  await _compartir(lista);
}

async function compartirLista(){
  const listas=getListas();
  if(listas.length===1){await _compartir(listas[0]);return;}
  // Mostrar selector visual
  const nombres=listas.map((l,i)=>`${i+1}. ${l.nombre} (${l.items.length} titulos)`).join('
');
  const resp=prompt(`Que lista quieres compartir?
...
Escribe el numero:`);
  ...
  await _compartir(listas[idx]);
}

async function _compartir(lista){
  const alias=aliasActual||'Seriestopia';
  // Solo guardamos id + titulo + poster para URL corta
  const listaCompacta=lista.items.map(i=>({id:i.id,title:i.title||i.name,poster_path:i.poster_path,vote_average:i.vote_average}));
  const payload={alias,nombre:lista.nombre,lista:listaCompacta,fecha:new Date().toISOString()};
  ...
  try{
    ...
    if(u.startsWith('http')&&!u.includes('error'))urlFinal=u;
  }catch{}
  const titulo=lista.nombre;
  const texto=`${alias} comparte su lista "${titulo}" en Seriestopia - ${lista.items.length} titulos`;
  // Web Share API (movil / redes sociales nativas)
  if(navigator.share){
    try{await navigator.share({title:`Lista: ${titulo}`,text:texto,url:urlFinal});return;}
    catch(e){if(e.name==='AbortError')return;}
  }
  // Fallback: modal bonito
  _mostrarModalCompartir(titulo,urlFinal,texto);
}

function _mostrarModalCompartir(titulo,url,texto){
  const prev=document.getElementById('_modalCompartir');if(prev)prev.remove();
  const overlay=document.createElement('div');
  overlay.id='_modalCompartir';
  overlay.style.cssText='position:fixed;inset:0;z-index:9998;background:rgba(0,0,0,0.8);display:flex;align-items:center;justify-content:center;padding:1rem;';
  overlay.innerHTML=`<div style="background:#1a1a2e;border:2px solid #e74c3c;border-radius:18px;padding:1.8rem;width:100%;max-width:420px;text-align:center;">
    <h3 style="color:#ffd700;margin-bottom:0.5rem">Compartir lista</h3>
    <p style="color:#b0b0b0;font-size:0.9rem;margin-bottom:1.2rem">${escapeHtml(titulo)}</p>
    <input id="_urlCompartir" value="${escapeHtml(url)}" readonly style="width:100%;padding:10px;border-radius:8px;border:1px solid #e74c3c;background:#0f0f23;color:#fff;font-size:0.85rem;text-align:center;margin-bottom:1rem;">
    <div style="display:flex;gap:8px;justify-content:center;flex-wrap:wrap;margin-bottom:1rem;">
      <button onclick="_copiarURL()" style="padding:8px 18px;background:#e74c3c;border:none;border-radius:20px;color:white;cursor:pointer;font-weight:bold;">Copiar enlace</button>
      <a href="https://wa.me/?text=${encodeURIComponent(texto+' '+url)}" target="_blank" style="padding:8px 18px;background:#25D366;border:none;border-radius:20px;color:white;cursor:pointer;font-weight:bold;text-decoration:none;">WhatsApp</a>
      <a href="https://twitter.com/intent/tweet?text=${encodeURIComponent(texto)}&url=${encodeURIComponent(url)}" target="_blank" style="padding:8px 18px;background:#1DA1F2;border:none;border-radius:20px;color:white;text-decoration:none;font-weight:bold;">Twitter/X</a>
    </div>
    <button onclick="document.getElementById('_modalCompartir').remove()" style="padding:7px 20px;background:rgba(255,255,255,0.1);border:1px solid rgba(255,255,255,0.2);border-radius:20px;color:white;cursor:pointer;">Cerrar</button>
  </div>`;
  document.body.appendChild(overlay);
  overlay.addEventListener('click',e=>{if(e.target===overlay)overlay.remove();});
  setTimeout(()=>{const inp=document.getElementById('_urlCompartir');if(inp){inp.focus();inp.select();}},100);
}

function _copiarURL(){
  const inp=document.getElementById('_urlCompartir');if(!inp)return;
  navigator.clipboard.writeText(inp.value)
    .then(()=>mostrarNotificacion('Enlace copiado','success'))
    .catch(()=>{try{inp.select();document.execCommand('copy');mostrarNotificacion('Enlace copiado','success');}catch{}});
}
...
function cargarPerfil(){
  ...
  const bioEl=document.getElementById('bioInput');if(bioEl)bioEl.value=localStorage.getItem('bio')||'';
  ...
  else if(sp){sp.innerHTML=emoji;sp.style.display='flex';if(img)img.style.display='none';}
}
...
function actualizarStatsPerfil(){
  ...
  if(el('statsMiLista'))el('statsMiLista').textContent=totalItems;
  if(el('statsRecordatorios'))el('statsRecordatorios').textContent=rec.length;
  if(el('statsPuntuadas'))el('statsPuntuadas').textContent=totalPunt;
  if(el('statsListas'))el('statsListas').textContent=listas.length;
}
...
// ============================================
const GENEROS_TMDB=[
  {id:28,nombre:'Accion'},{id:12,nombre:'Aventura'},{id:16,nombre:'Animacion'},
  {id:35,nombre:'Comedia'},{id:80,nombre:'Crimen'},{id:99,nombre:'Documental'},
  {id:18,nombre:'Drama'},{id:10751,nombre:'Familia'},{id:14,nombre:'Fantasia'},
  {id:27,nombre:'Terror'},{id:9648,nombre:'Misterio'},{id:10749,nombre:'Romance'},
  {id:878,nombre:'Ciencia Ficcion'},{id:53,nombre:'Thriller'},{id:10752,nombre:'Belico'},
  {id:10759,nombre:'Accion y Aventura'},{id:10765,nombre:'Sci-Fi y Fantasia'},{id:37,nombre:'Western'}
];
const PLATAFORMAS_PREF=[
  {id:8,nombre:'Netflix'},{id:337,nombre:'Disney+'},{id:1899,nombre:'Max'},
  {id:119,nombre:'Prime Video'},{id:350,nombre:'Apple TV+'},{id:149,nombre:'Movistar+'},
  {id:63,nombre:'Filmin'},{id:531,nombre:'Paramount+'},{id:1773,nombre:'SkyShowtime'}
];
let prefTipoActual='ambos';
let paratiTabActual='tv';

function getPreferencias(){try{return JSON.parse(localStorage.getItem('preferencias')||'null');}catch{return null;}}

function mostrarSeccionParaTi(){
  const pref=getPreferencias();
  const onb=document.getElementById('paratiOnboarding');
  const res=document.getElementById('paratiResultados');
  if(!pref||!pref.generos||!pref.generos.length){
    if(onb)onb.style.display='block';if(res)res.style.display='none';
    renderOnboarding();
  }else{
    if(onb)onb.style.display='none';if(res)res.style.display='block';
    cargarRecomendaciones(pref);
  }
}

function mostrarOnboarding(){
  const onb=document.getElementById('paratiOnboarding');const res=document.getElementById('paratiResultados');
  if(onb)onb.style.display='block';if(res)res.style.display='none';
  renderOnboarding();
}
...
function renderOnboarding(){
  ...
  if(gGrid){
    gGrid.innerHTML='';
    GENEROS_TMDB.forEach(g=>{
      const btn=document.createElement('button');
      btn.className='genero-btn'+(pref.generos.includes(g.id)?' selected':'');
      btn.textContent=g.nombre;btn.dataset.id=g.id;
  ...
  if(pGrid){
    ...
    PLATAFORMAS_PREF.forEach(p=>{
      ...
      btn.textContent=p.nombre;btn.dataset.id=p.id;
      btn.onclick=()=>btn.classList.toggle('selected');
      pGrid.appendChild(btn);
    });
  }
  prefTipoActual=pref.tipo||'ambos';
  document.querySelectorAll('.tipo-pref-btn').forEach(b=>b.classList.toggle('active',b.dataset.tipo===prefTipoActual));
}
...
function guardarPreferencias(){
  const generos=Array.from(document.querySelectorAll('.genero-btn.selected')).map(b=>parseInt(b.dataset.id));
  const plataformas=Array.from(document.querySelectorAll('.plataforma-pref-btn.selected')).map(b=>parseInt(b.dataset.id));
  if(!generos.length){mostrarNotificacion('Selecciona al menos un genero','error');return;}
  const pref={generos,plataformas,tipo:prefTipoActual};
...
async function cargarRecomendaciones(pref){
  let tipo=paratiTabActual;
  if(pref.tipo==='tv')tipo='tv';
  if(pref.tipo==='movie')tipo='movie';
  const tabS=document.getElementById('tabSeries');const tabP=document.getElementById('tabPeliculas');
  if(tabS)tabS.classList.toggle('active',tipo==='tv');
  if(tabP)tabP.classList.toggle('active',tipo==='movie');
  ...
  const ck=`parati_${tipo}_${pref.generos.join('-')}_${pref.plataformas.join('-')}`;
  const cached=localStorage.getItem(ck);
  if(cached){try{const c=JSON.parse(cached);if(Date.now()-c.time<3600000){ocultarLoader('paratiContainer');mostrarResultadosConLogos(c.data,'paratiContainer');return;}}catch{}}
  try{
    const providers=pref.plataformas.length?pref.plataformas.join('|'):'';
    const baseUrl=`${BASEURL}discover/${tipo}?api_key=${APIKEY}&language=es-ES&watch_region=ES&with_genres=${pref.generos.join('|')}&sort_by=popularity.desc&vote_count.gte=50${providers?'&with_watch_providers='+providers:''}`;
    const [d1,d2]=await Promise.all([fetch(baseUrl+'&page=1').then(r=>r.json()),fetch(baseUrl+'&page=2').then(r=>r.json())]);
    const results=[...(d1.results||[]),...(d2.results||[])];
    const items=await Promise.all(results.slice(0,40).map(i=>enriquecerConPlataformasTMDb(i,tipo)));
    localStorage.setItem(ck,JSON.stringify({time:Date.now(),data:items}));
...
function cambiarTabParaTi(tipo){
  ...
  const pref=getPreferencias();
  if(pref)cargarRecomendaciones(Object.assign({},pref,{tipo}));
}
...
(function(){
  ...
  window.addEventListener('scroll',function(){
    ...
    if(y>lastY&&y>80)h.classList.add('header-oculto');
